...
class MT(nn.Module):
    ...
    def __init__(
        self, nc=80, anchors=(), attn=None, mask_iou=False, ch=()
    ):  # detection layer
        ...
        self.m = nn.ModuleList(
            nn.Conv2d(x, self.no * self.na, 1) for x in ch[0]
        )  # output conv
        if mask_iou:
            self.m_iou = nn.ModuleList(
                nn.Conv2d(x, self.na, 1) for x in ch[0]
            )  # output con
        self.mask_iou = mask_iou
        self.attn = attn
        if attn is not None:
            # self.attn_m = nn.ModuleList(nn.Conv2d(x, attn * self.na, 3, padding=1) for x in ch)  # output conv
            self.attn_m = nn.ModuleList(
                nn.Conv2d(x, attn * self.na, 1) for x in ch[0]
            )  # output conv
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.attn is not None:
                ...
                attn[i] = (
                    attn[i]
                    .view(bs, self.na, self.attn, ny, nx)
                    .permute(0, 1, 3, 4, 2)
                    .contiguous()
                )
            if self.mask_iou:
            ...
            if not self.training:  # inference
                ...
                y = x[0][i].sigmoid()
                y[..., 0:2] = (y[..., 0:2] * 3.0 - 1.0 + self.grid[i]) * self.stride[
                    i
                ]  # xy
                y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                z.append(y.view(bs, -1, self.no))
    ...
    def __init__(
        ...
        if isinstance(m, MT):
            ...
            if isinstance(temp, list):
                temp = temp[0]
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in temp["bbox_and_cls"]]
            )  # forward
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)
            self.stride = m.stride
