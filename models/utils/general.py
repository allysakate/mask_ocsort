...
def non_max_suppression_mask_conf(
    prediction,
    attn,
    bases,
    pooler,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if mask_iou is not None:
            ...
            mask_score = torch.exp(
                torch.log(temp).mean(dim=-1, keepdims=True)
            )  # torch.mean(temp, dim=-1, keepdims=True)

        x[:, 5:] *= (
            x[:, 4:5] * mask_score
        )  # x[:, 4:5] *   * mask_conf * non_mask_conf  # conf = obj_conf * cls_conf
