    ...
    for x in args:
        ...
        if type_name in ["Conv2d"]:
            out_h = int(
                (
                    x.size()[2]
                    + 2 * layer.padding[0] / layer.dilation[0]
                    - layer.kernel_size[0]
                )
                / layer.stride[0]
                + 1
            ...
            out_w = int(
                (
                    x.size()[3]
                    + 2 * layer.padding[1] / layer.dilation[1]
                    - layer.kernel_size[1]
                )
                / layer.stride[1]
                + 1
            ...
            out_h = int(
                (in_h - 1) * layer.stride[0]
                - 2 * layer.padding[0]
                + layer.kernel_size[0]
                + layer.output_padding[0]
            )
            out_w = int(
                (in_w - 1) * layer.stride[1]
                - 2 * layer.padding[1]
                + layer.kernel_size[1]
        ...
        # ops_learned_conv
            ...
            conv = layer.conv
            out_h = int(
                (x.size()[2] + 2 * conv.padding[0] - conv.kernel_size[0])
                / conv.stride[0]
                + 1
            )
            out_w = int(
                (x.size()[3] + 2 * conv.padding[1] - conv.kernel_size[1])
                / conv.stride[1]
                + 1
            )
            delta_ops = (
        ...
        # ops_pooling
            ...
            out_w = int(
                (in_w + 2 * layer.padding - layer.kernel_size) / layer.stride + 1
            )
            out_h = int(
                (in_w + 2 * layer.padding - layer.kernel_size) / layer.stride + 1
            )
        ...
        # ops_nothing
            ...
            delta_ops = reduce(
                operator.mul, offset_conv.size(), x.size()[2] * x.size()[3]
            )
            out_h = int(
                (
                    x.size()[2]
