...
from models.utils.comm import reduce_mean, compute_ious
from models.layers import ml_nms, IOULoss
...
class FCOSOutputs(nn.Module):
    ...
    def _get_ground_truth(self, locations, gt_instances):
        ...
        loc_to_size_range = []
        for loc, loc_per_level in enumerate(locations):
    ...
    def predict_proposals(
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            location = per_bundle["l"]
            log_pred = per_bundle["o"]
            rgr_pred = per_bundle["r"] * per_bundle["s"]
            ctr_pred = per_bundle["c"]
            top_feat = per_bundle["t"] if "t" in bundle else None

            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    location, log_pred, rgr_pred, ctr_pred, image_sizes, top_feat
                )
            )

            for per_im_sampled_boxes in sampled_boxes[-1]:
                per_im_sampled_boxes.fpn_levels = (
                    location.new_ones(len(per_im_sampled_boxes), dtype=torch.long) * i
                )
