    ...
    def forward(self, features, targets=None):
        for i, f in enumerate(self.in_features):
            if i == 0:
                ...
                x_p = F.interpolate(
                    x_p, x.size()[2:], mode="bilinear", align_corners=False
                )
        ...
        if self.training and self.loss_on:
            ...
            gt_sem = F.interpolate(gt_sem, scale_factor=1 / self.common_stride, recompute_scale_factor=False)
            seg_loss = F.cross_entropy(sem_out, gt_sem.squeeze(1).long())
