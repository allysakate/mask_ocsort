    ...
    def __init__(self, cfg):
        ...
        if self.combine_on:
            ...
            self.combine_overlap_threshold = (
                cfg.MODEL.PANOPTIC_FPN.COMBINE.OVERLAP_THRESH
            )
            self.combine_stuff_area_limit = (
                cfg.MODEL.PANOPTIC_FPN.COMBINE.STUFF_AREA_LIMIT
            )
        ...
        self.top_layer = nn.Conv2d(
            in_channels, attn_len, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, batched_inputs):
        ...
        if self.combine_on:
            if "sem_seg" in batched_inputs[0]:
                ...
                gt_sem = ImageList.from_tensors(
                    ...
                    self.panoptic_module.ignore_value,
                ).tensor
            else:
                gt_sem = None
        ...
        if "basis_sem" in batched_inputs[0]:
            ...
            basis_sem = ImageList.from_tensors(
                basis_sem, self.backbone.size_divisibility, 0
            ).tensor
        else:
            basis_sem = None
