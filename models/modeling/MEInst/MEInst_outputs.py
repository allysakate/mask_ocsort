...
from models.utils.comm import reduce_sum
from models.layers import ml_nms
...
class MEInstOutputs(object):
    ...
    def prepare_masks(
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            if not len(proposals_per_image):
                gt_boxes = Boxes(targets_per_image.gt_boxes.tensor.new_zeros((0, 4)))
    ...
    def _get_ground_truth(self):
        ...
        loc_to_size_range = []
        for loc, loc_per_level in enumerate(self.locations):
            loc_to_size_range_per_level = loc_per_level.new_tensor(
                self.sizes_of_interest[loc]
            )
            loc_to_size_range.append(
                loc_to_size_range_per_level[None].expand(num_loc_list[loc], -1)
            )
        ...
        for mask_target_per_img, mask_index_per_img in zip(mask_targets, mask_indices):
            ...
            for level_e in num_loc_list:
                ...
                mask_target_per_level = (
                    mask_target_per_img[index_level]
        ...
        for level in range(len(self.locations)):
            mask_targets_level_first.append(
                torch.cat(
                    [mask_per_im[level] for mask_per_im in mask_targets_split], dim=0
                )
            )
        ...
        for labels_per_level, mask_targets_level in zip(
            labels_level_first, mask_targets_level_first
        ):
            num_pos = (labels_per_level != self.num_classes).nonzero().numel()
            assert num_pos == mask_targets_level.shape[0], print(
                "The number(positive) should be equal between labels_per_level and mask_targets_level."
            )
        ...
        for target in range(len(reg_targets)):
            reg_targets[target] = reg_targets[target] / float(self.strides[target])
    ...
    def MEInst_losses(
        ...
        assert mask_pred.shape[0] == mask_targets.shape[0], print(
            "The number(positive) should be equal between "
        ...
        reg_loss = self.iou_loss(reg_pred, reg_targets, ctrness_targets) / ctrness_norm
    ...
    def losses(self):
        ...
        labels, reg_targets, mask_targets = (
        ...
        mask_targets = cat(
            [
                ...
                x.reshape(-1, self.mask_size**2)
                for x in mask_targets
            ],
            dim=0,
        )
    ...
    def predict_proposals(self):
        ...
        for i, (l, o, r, c, s, mr) in enumerate(zip(*bundle)):
            ...
            sampled_boxes.append(
                self.forward_for_single_feature_map(l, o, r, c, mr, self.image_sizes)
            )
        ...
        for i in range(num_images):
            ...
            per_image_masks = self.mask_encoding.decoder(
                per_image_masks, is_train=False
            )
            per_image_masks = per_image_masks.view(
    ...
    def forward_for_single_feature_map(
        ...
        mask_regression = mask_regression.view(N, self.dim_mask, H, W).permute(
            0, 2, 3, 1
        )
