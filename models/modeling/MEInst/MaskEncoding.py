...
class PCAMaskEncoding(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.agnostic:
            self.components = nn.Parameter(
                torch.zeros(self.dim_mask, self.mask_size**2), requires_grad=False
            )
            self.explained_variances = nn.Parameter(
                torch.zeros(self.dim_mask), requires_grad=False
            )
            self.means = nn.Parameter(
                torch.zeros(self.mask_size**2), requires_grad=False
            )
        else:
    ...
    def inverse_sigmoid(self, x):
        ...
        value_random = torch.where(
            value_random > VALUE_MIN, value_random, VALUE_MIN * torch.ones_like(x)
        )
    ...
    def encoder(self, X):
        ...
        """
        assert X.shape[1] == self.mask_size**2, print(
            "The original mask_size of input" " should be equal to the supposed size."
        )
    ...
    def decoder(self, X, is_train=False):
        ...
        """
        assert X.shape[1] == self.dim_mask, print(
            "The dim of transformed data " "should be equal to the supposed dim."
        )

        if self.agnostic:
            if self.whiten:
                components_ = self.components * torch.sqrt(
                    self.explained_variances.unsqueeze(1)
                )
