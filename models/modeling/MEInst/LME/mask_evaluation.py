...
def parse_args():
    ...
    parser.add_argument(
        "--matrix",
        default="coco/components/coco_2017_train"
        "_class_agnosticTrue_whitenTrue_sigmoidTrue_60.npz",
        type=str,
    )
...
if __name__ == "__main__":
    ...
    for i, masks in enumerate(mask_loader):
        ...
        if sigmoid:
            ...
            masks_random = np.where(
                masks > value_random, 1 - value_random, value_random
            )
        ...
        mask_rc = transform(
            masks_random,
            components_=components_c,
            explained_variance_=explained_variance_c,
            mean_=mean_c,
            whiten=whiten,
        )
        mask_rc = inverse_transform(
            mask_rc,
            components_=components_c,
            explained_variance_=explained_variance_c,
            mean_=mean_c,
