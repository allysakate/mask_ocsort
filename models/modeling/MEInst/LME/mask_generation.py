...
def mask_encoding(
    ...
    if class_agnostic:
        ...
        pca = IncrementalPCA(
            n_components=n_components, copy=False, whiten=whiten, batch_size=batch_size
        )
        pca.fit(masks)
...
if __name__ == "__main__":
    ...
    mask_loader = DataLoader(
        mask_data, batch_size=args.batch_size, shuffle=False, num_workers=4
    )
    ...
    tic = time.time()
    components_c, mean_c, ratio_c, explained_variance_c, ratio = mask_encoding(
        masks, n_components, class_agnostic, whiten, sigmoid, args.batch_size
    )
    toc = time.time() - tic
    print("Finish the mask encoding in {}s.".format(toc))

    components_c = (
        np.concatenate(components_c).mean(0)[np.newaxis, :, :].astype(np.float32)
    )
    ...
    explained_variance_c = (
        np.concatenate(explained_variance_c).mean(0)[np.newaxis, :].astype(np.float32)
    )
