...
from models.utils.comm import reduce_sum, compute_ious
...
def fcos_losses(
    ...
    bezier_loss = F.smooth_l1_loss(bezier_pred, bezier_targets, reduction="none")
    bezier_loss = (bezier_loss.mean(dim=-1) * ctrness_targets).sum() / loss_denorm
...
class BATextOutputs(object):
    ...
    def _get_ground_truth(self):
        ...
        loc_to_size_range = []
        for layer, loc_per_level in enumerate(self.locations):
            loc_to_size_range_per_level = loc_per_level.new_tensor(
                self.sizes_of_interest[layer]
            )
            loc_to_size_range.append(
                loc_to_size_range_per_level[None].expand(num_loc_list[layer], -1)
            )
        ...
        for layer in range(len(reg_targets)):
            reg_targets[layer] = reg_targets[layer] / float(self.strides[layer])
            bezier_targets[layer] = bezier_targets[layer] / float(self.strides[layer])
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges):
        ...
        for im_i in range(len(targets)):
            ...
            bezier_targets_per_im = bezier_targets_per_im.view(
                xs.size(0), bboxes.size(0), 16
            )
            ...
            bezier_targets_per_im = bezier_targets_per_im[
                range(len(locations)), locations_to_gt_inds
            ]
    ...
    def predict_proposals(self, top_feats):
        ...
        for i, instance in enumerate(zip(*bundle.values())):
            ...
            location = instance_dict["l"]
            log_pred = instance_dict["o"]
            rgr_pred = instance_dict["r"] * instance_dict["s"]
            ctr_pred = instance_dict["c"]
            # top_feat is the bezier regression
            top_feat = (
                instance_dict["t"] * instance_dict["s"] if "t" in bundle else None
            )

            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    location, log_pred, rgr_pred, ctr_pred, self.image_sizes, top_feat
                )
            )
    ...
    def forward_for_single_feature_map(
        ...
        for i in range(N):
            ...
            if top_feat is not None:
                ...
                # decentralize
                bezier_detections = per_locations.unsqueeze(1) + per_top_feat.view(
                    -1, 8, 2
                )
