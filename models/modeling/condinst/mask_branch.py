...
from models.utils.comm import aligned_bilinear
...
class MaskBranch(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        for in_feature in self.in_features:
            self.refine.append(conv_block(feature_channels[in_feature], channels, 3, 1))
    ...
    def forward(self, features, gt_instances=None):
        ...
        if self.training and self.sem_loss_on:
            ...
            class_range = torch.arange(
                num_classes, dtype=logits_pred.dtype, device=logits_pred.device
            )[:, None, None]
