...
from models.utils.comm import compute_locations, aligned_bilinear
...
def compute_pairwise_term(mask_logits, pairwise_size, pairwise_dilation):
    ...
    from models.modeling.condinst.condinst import unfold_wo_center
...
def parse_dynamic_params(params, channels, weight_nums, bias_nums):
    ...
    for layer in range(num_layers):
        if layer < num_layers - 1:
            ...
            bias_splits[layer] = bias_splits[layer].reshape(num_insts * channels)
        else:
            # out_channels x in_channels x 1 x 1
            weight_splits[layer] = weight_splits[layer].reshape(num_insts * 1, -1, 1, 1)
...
class DynamicMaskHead(nn.Module):
    ...
    def mask_heads_forward_with_coords(self, mask_feats, mask_feat_stride, instances):
        ...
        if not self.disable_rel_coords:
            ...
            relative_coords = instance_locations.reshape(-1, 1, 2) - locations.reshape(
                1, -1, 2
            )
            ...
            mask_head_inputs = torch.cat(
                [
                    relative_coords,
                    mask_feats[im_inds].reshape(n_inst, self.in_channels, H * W),
                ],
                dim=1,
            )
        else:
