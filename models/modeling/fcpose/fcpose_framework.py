...
from models.modeling.fcos import FCOS
from .basis_module import basis_module
from .fcpose_head import fcpose_head_module
from .utils import top_module, process_gt_instances
...
class FCPose(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None):
        if gt_instances is not None:
            basis_gt_heatmap, head_gt_heatmap, p3_heatmap_list = process_gt_instances(
                gt_instances, self.gt_stride, self.device
            )
        else:
            basis_gt_heatmap, head_gt_heatmap, p3_heatmap_list = None, None, None

        proposals, proposal_losses = self.fcos(
            images, features, gt_instances, self.top_module
        )

        basis_out, basis_losses = self.basis_module(
            features, basis_gt_heatmap, p3_heatmap_list
        )
