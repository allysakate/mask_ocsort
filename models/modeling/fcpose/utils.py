...
def process_gt_instances(gt_instances, gt_stride, device):
    ...
    for instances in gt_instances:
        one_frame_instances = instances.keypoint_heatmap.to(
        ...
        one_frame_instances = instances.head_heatmap.to(
            device=device, dtype=torch.float
        )
    ...
    return (
        basis_heatmap_list.tensor,
        head_heatmap_list.tensor.bool(),
        p3_heatmap_list.tensor,
    )
...
def compute_loss_softmax(
    gt_bitmasks,
    mask_logits,
    num_loss,
    num_instances,
    direction,
    direction_mask_logits,
    gt_keypoint,
    max_ranges,
    distance_norm,
):
    ...
    if gt_bitmasks_visible_mask.sum() != 0:
        ...
        max_ranges = (
            max_ranges[:, None].repeat(1, 17).flatten()[gt_bitmasks_visible_mask]
        )
        ...
        direction = (direction[:, :, :, 0] ** 2 + direction[:, :, :, 1] ** 2).sqrt()[
            :, :, :, None
        ]
        ...
        direction_mask_logits = direction_mask_logits.permute(0, 3, 1, 2, 4).reshape(
            N * 17, H, W, 1
        )
        ...
        direction_loss = (
            direction / sum_loss_weight * gpu_balence_factor
        ) / distance_norm
