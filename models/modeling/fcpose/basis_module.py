...
from models.layers import conv_with_kaiming_uniform
...
class basis_module(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        for in_feature in self.in_features:
            self.refine.append(conv_block(feature_channels[in_feature], planes, 3, 1))
        tower = []
        for i in range(num_convs):
            tower.append(conv_block(planes, planes, 3, 1))
        tower.append(conv_block(planes, planes, 3, 1))
        tower.append(nn.Conv2d(planes, mask_dim + (2 * 17), 1))
        ...
        if self.loss_on:
            ...
            self.p3_logits = nn.Conv2d(
                planes, self.num_classes, kernel_size=1, stride=1
            )
            self.upsampler = nn.Sequential(
                ConvTranspose2d(
                    planes + self.num_classes, planes, 8, stride=4, padding=6 // 2 - 1
                ),
                # get_norm(norm, planes),
                nn.ReLU(),
                # conv_block(planes, planes, 3,1),
            )
            self.p1_logits = nn.Conv2d(
                planes, self.num_classes, kernel_size=3, stride=1, padding=1
            )
