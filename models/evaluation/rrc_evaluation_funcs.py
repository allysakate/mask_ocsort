...
def load_zip_file_keys(file, fileNameRegExp=""):
    ...
    for name in archive.namelist():
        ...
        keyName = name
        if fileNameRegExp != "":
            match = re.match(fileNameRegExp, name)
            if not match:
                addFile = False
            else:
...
def validate_clockwise_points(points):
    ...
    #  have to be given in clockwise order. Regarding the correct interpretation of 'clockwise'
    #  remember that the image coordinate system used is the standard one, with the image origin at
    #  the upper left, the X axis extending to the right and Y axis extending downwards.")
    ...
    if pRing.is_ccw:
        message = "Points are not clockwise. The coordinates of bounding quadrilaterals"
        +"have to be given in clockwise order. Regarding the correct interpretation of 'clockwise' "
        +"remember that the image coordinate system used is the standard one, with the image origin at"
        +"the upper left, the X axis extending to the right and Y axis extending downwards."
        assert 0, message
...
def main_evaluation(
    ...
    """
    This process validates a method, evaluates it and if it succed generates
    ...
    default_evaluation_params_fn: points to a function that returns a dictionary with the
        default parameters used for the evaluation
    validate_data_fn: points to a method that validates the correct format of the submission
    evaluate_method_fn: points to a function that evaluated the submission
...
def main_validation(default_evaluation_params_fn, validate_data_fn):
    ...
    Params:
    default_evaluation_params_fn: points to a function that returns a dictionary with the default parameters used
    for the evaluation validate_data_fn: points to a method that validates the corrct format of the submission
    """
    try:
        p = dict([s[1:].split("=") for s in sys.argv[1:]])
        evalParams = default_evaluation_params_fn()
