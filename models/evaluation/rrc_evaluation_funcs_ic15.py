...
def get_tl_line_values(
    ...
    if LTRB:
        ...
        points = [float(match.group(i)) for i in range(1, (numPoints + 1))]
        ...
        if withTranscription and withConfidence:
            match = re.match(
                r"^\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*([0-1].?[0-9]*)\s*,(.*)$",  # noqa
                line,
            )
            if not match:
                raise Exception(
            ...
            match = re.match(
                r"^\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,\s*(-?[0-9]+)\s*,(.*)$",  # noqa
                line,
            )
            if not match:
                raise Exception(
    ...
    if withConfidence:
        try:
            confidence = float(match.group(numPoints + 1))
        except ValueError:
            raise Exception("Confidence value must be a float")

    if withTranscription:
        posTranscription = numPoints + (2 if withConfidence else 1)
        transcription = match.group(posTranscription)
...
def validate_clockwise_points(points):
    ...
    if summatory > 0:
        raise Exception(
            ...
            + "Regarding the correct interpretation of 'clockwise' remember that the image "
            + "coordinate system used is the standard one, with the image origin at the upper left, "
...
def main_evaluation(
    ...
    """
    This process validates a method, evaluates it and if it succed
    ...
    default_evaluation_params_fn: points to a function that returns
        a dictionary with the default parameters used for the evaluation
