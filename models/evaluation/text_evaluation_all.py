...
from models.evaluation import text_eval_script
from models.evaluation import text_eval_script_ic15
import zipfile
import pickle
import editdistance
import cv2
...
class TextEvaluator:
    ...
    def sort_detection(self, temp_dir):
        ...
        if "totaltext" in self.dataset_name:
            ...
            if self.lexicon_type == 1:
                ...
                lexicon_fid = open(lexicon_path, "r")
                pair_list = open(
                    "datasets/icdar2015/GenericVocabulary_pair_list.txt", "r"
                )
                pairs = dict()
                for line in pair_list.readlines():
                    line = line.strip()
            ...
            if self.lexicon_type == 2:
                ...
                lexicon_fid = open(lexicon_path, "r")
                pair_list = open(
                    "datasets/icdar2015/ch4_test_vocabulary_pair_list.txt", "r"
                )
                pairs = dict()
                for line in pair_list.readlines():
                    line = line.strip()
        ...
        for i in files:
            if "icdar2015" in self.dataset_name:
                ...
                if self.lexicon_type == 3:
                    lexicon_path = (
                        "datasets/icdar2015/new_strong_lexicon/new_voc_img_"
                        + str(int(i.split("/")[-1].split(".")[0]))
                        + ".txt"
                    )
                    lexicon_fid = open(lexicon_path, "r")
                    pair_list = open(
                        "datasets/icdar2015/new_strong_lexicon/pair_voc_img_"
                        + str(int(i.split("/")[-1].split(".")[0]))
                        + ".txt"
                    )
                    pairs = dict()
            ...
            for iline, line in enumerate(fin):
                ...
                if "icdar2015" not in self.dataset_name:
                    if pRing.is_ccw:
                        pts.reverse()
                outstr = ""
                for ipt in pts[:-1]:
    ...
    def evaluate(self):
        ...
        text_result_full["e2e_method"] = (
            dict_lexicon[str(self.lexicon_type)] + "-" + text_result_full["e2e_method"]
        )
        os.remove(result_path)
        os.remove(result_path_full)
        # parse
        template = r"(\S+): (\S+): (\S+), (\S+): (\S+), (\S+): (\S+)"
